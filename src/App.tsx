...
import type { Video, Comment, User, SiteSettings } from './types';
...
const EstherPlatform = () => {
  ...
  const loadData = async () => {
    try {
      const [videosResult, settingsResult, commentsResult] = await Promise.all([
  ...
  const saveAllChanges = async () => {
    ...
    try {
      for (const video of videos) {
        const { comments, ...videoData } = video;
        await supabase
          .from('videos')
          .upsert({
            ...videoData,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        <div className="relative min-h-screen flex items-center justify-center overflow-hidden py-12 px-4">
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/20 to-pink-900/20 animate-pulse-slow"></div>
            ...
            {completedVideos.length === 0 ? (
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {completedVideos.slice(0, 6).map((video, idx) => (
                  <div
                    key={video.id}
                    className="bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl hover:shadow-purple-500/20 hover:scale-105 transition-all duration-300 animate-fade-in"
                  ...
                  >
                    <div className="aspect-video bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center overflow-hidden">
                      ...
                      <Music size={64} className={`text-white ${video.thumbnail_url || extractGoogleDriveThumbnail(video.drive_link) ? 'hidden' : ''}`} />
                    </div>
                    <div className="p-6">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
          ...
          <div className="grid gap-4 md:gap-6 xl:grid-cols-2">
            {localVideos.map((video, idx) => (
              <div
                key={video.id}
                className="bg-gray-800 border border-gray-700 rounded-xl shadow-lg hover:shadow-purple-500/20 transition-all duration-300 p-4 md:p-5 animate-fade-in hover:scale-[1.02]"
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl p-5 sm:p-6 md:p-8 max-w-2xl w-full my-8 animate-slide-up max-h-[90vh] overflow-y-auto scrollbar-thin">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={localSettings.profile_image}
                  onChange={(e) => setLocalSettings({...localSettings, profile_image: e.target.value})}
                  placeholder="/Estherreign.jpg"
                  className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 text-white transition-all"
                />
                <p className="text-xs text-gray-500 mt-1">Upload image to /public folder and enter path here</p>
              </div>
